...
function startRecursive(i = 0, total = 0) {
  return i >= users.length ? 
    Promise.resolve(total) :
    Promise.all(users.slice(i, i + 10).map(user => processUser(user.id)))
      .then(result =>
        startRecursive(i + 10, (total + result.reduce((acc, item) => acc + item)))
    )
}
